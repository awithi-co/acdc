...
test('Codex sends to Claude without exposing a reverse delivery endpoint',async()=>{
 ...
 try {
  ...
  const claudeTools=await b.client.listTools();assert.deepEqual(claudeTools.tools.map(t=>t.name).sort(),['list_peers','register_peer','send_message']);
  ...
  assert.equal((await a.call('register_peer',{name:'codex',thread_id:'11111111-1111-1111-1111-111111111111'})).ready,false);
