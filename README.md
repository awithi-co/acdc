...
The [ACDC stdio relay](RELAY.md) lets Codex discover running Claude recipients and send context through Claude channel notifications. Opt-in receiving uses a local Codex app-server and native `codex queue` to push Claude replies into the selected Codex conversation. Ordinary Codex sessions remain send-only. ACDC stores no message inbox or history. The MCP runtime requires uv/Python 3.11+; existing session-reading skills remain independent.
