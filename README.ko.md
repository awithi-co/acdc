...
한 코딩 에이전트(Claude Code 또는 Codex)에서 한창 작업하던 중에 흐름이 끊깁니다. Rate limit이 걸리거나, 컨텍스트 한도가 차거나, 다음 작업은 다른 에이전트가 더 잘할 것 같거나.

또는, 분명히 어디선가 이 얘기를 했던 것 같은데 어디였는지 찾을 수 없는 경우 — 지난주 다른 세션에서 다뤘거나, 또는 현재 세션의 compact 이후 에이전트가 살짝 바보가 돼서 앞부분을 흐릿하게 기억하거나.

ACDC는 두 상황 모두를 로컬 세션 저장소 위에서 처리합니다:

- **인계** — ACDC가 이전 세션과 현재 레포 상태를 읽은 다음, 구조화된 요약을 새 에이전트에게 넘겨서 마지막 몇 메시지를 그대로 되풀이하는 게 아니라 실제로 작업을 이어가게 합니다.
- **회상** — 본인의 과거 세션을 이름이나 UUID로 찾아서, 무엇을 결정했고, 무엇이 바뀌었고, 어디서 끊겼는지 타임라인 요약으로 받습니다.
- **검색** — 세션 간을 토픽으로 검색합니다. 지난주 어디서 그 얘기 했었는지, 또는 현재 세션에서 compact가 삼킨 앞부분까지.
...
(자가 회상과 토픽 검색은 에이전트별 — Claude는 자신의 트랜스크립트, Codex는 자신의 rollout을 검색합니다.)
