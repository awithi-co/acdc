...
uv run python <skill-dir>/../recall-session/scripts/find_claude_session.py <name-or-id> --json

# Codex sessions
uv run python <skill-dir>/../resume-codex-session/scripts/find_codex_session.py <name-or-id> --json
```
...
```bash
uv run python <skill-dir>/../recall-session/scripts/summarize_claude_transcript.py "$TRANSCRIPT" --tail 0 --max-events 40
uv run python <skill-dir>/../recall-session/scripts/summarize_claude_transcript.py "$TRANSCRIPT" --tail 120

# Codex rollouts hold more than one working session — map first, then read one.
uv run python <skill-dir>/../resume-codex-session/scripts/summarize_codex_rollout.py "$ROLLOUT" --segments
uv run python <skill-dir>/../resume-codex-session/scripts/summarize_codex_rollout.py "$ROLLOUT" --segment last
```
